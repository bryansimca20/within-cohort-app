...
test('two saves same member+day create two rows (insert, not upsert)', async () => {
  ...
  await saveSession(db, m, valid, new Date('2026-08-01T02:00:00Z')); // 09:00 Jakarta, day 0
  await saveSession(db, m, valid, new Date('2026-08-01T05:00:00Z')); // same local day
...
test('phase stamped baseline on day 0 and within on day 14', async () => {
  const { db } = await makeTestDb();
  const [m] = await db.insert(members).values({ name: 'Ana', passcodeHash: 'x', inCohort: true, isAdmin: false, cohortStartDate: '2026-08-01', timezone: 'Asia/Jakarta' }).returning();
  await saveSession(db, m, valid, new Date('2026-08-01T02:00:00Z')); // day 0 -> baseline
  await saveSession(db, m, valid, new Date('2026-08-15T02:00:00Z')); // day 14 -> within
...
test('tookServing is forced null in baseline even when input is true, and preserved in within', async () => {
  const { db } = await makeTestDb();
  const [m] = await db.insert(members).values({ name: 'Ana', passcodeHash: 'x', inCohort: true, isAdmin: false, cohortStartDate: '2026-08-01', timezone: 'Asia/Jakarta' }).returning();
  await saveSession(db, m, { ...valid, tookServing: true }, new Date('2026-08-01T02:00:00Z')); // baseline
  await saveSession(db, m, { ...valid, tookServing: true }, new Date('2026-08-15T02:00:00Z')); // within
  const rows = await db.select().from(sessionLogs).orderBy(sessionLogs.localDate);
  expect(rows[0].phase).toBe('baseline');
...
test('tookServing omitted stores null in within phase too', async () => {
  const { db } = await makeTestDb();
  const [m] = await db.insert(members).values({ name: 'Ana', passcodeHash: 'x', inCohort: true, isAdmin: false, cohortStartDate: '2026-08-01', timezone: 'Asia/Jakarta' }).returning();
  await saveSession(db, m, valid, new Date('2026-08-15T02:00:00Z')); // within, no tookServing given
  const rows = await db.select().from(sessionLogs);
  expect(rows[0].tookServing).toBeNull();
});
...
test('distanceKm round-trips through numeric(4,1)', async () => {
  const { db } = await makeTestDb();
  const [m] = await db.insert(members).values({ name: 'Ana', passcodeHash: 'x', inCohort: true, isAdmin: false, cohortStartDate: '2026-08-01', timezone: 'Asia/Jakarta' }).returning();
  await saveSession(db, m, valid, new Date('2026-08-01T02:00:00Z'));
  const rows = await db.select().from(sessionLogs);
...
test('rejects before cohort start', async () => {
  ...
  await expect(saveSession(db, m, valid, new Date('2026-08-01T02:00:00Z'))).rejects.toThrow(/not started/i);
});

test('rejects when cohort has not been assigned a start date', async () => {
  const { db } = await makeTestDb();
  const [m] = await db.insert(members).values({ name: 'Ana', passcodeHash: 'x', inCohort: true, isAdmin: false, cohortStartDate: null, timezone: 'Asia/Jakarta' }).returning();
  await expect(saveSession(db, m, valid, new Date('2026-08-01T02:00:00Z'))).rejects.toThrow(/not started/i);
});
...
test('rejects sessionType other without sessionTypeOther', async () => {
  const { db } = await makeTestDb();
  const [m] = await db.insert(members).values({ name: 'Ana', passcodeHash: 'x', inCohort: true, isAdmin: false, cohortStartDate: '2026-08-01', timezone: 'Asia/Jakarta' }).returning();
  await expect(saveSession(db, m, { ...valid, sessionType: 'other' }, new Date('2026-08-01T02:00:00Z'))).rejects.toThrow();
});
