...
test('flags who is missing today', async () => {
  ...
  await db
    .insert(members)
    .values({ name: 'Bo', passcodeHash: 'x', inCohort: true, isAdmin: false, cohortStartDate: '2026-08-01', timezone: 'Asia/Jakarta' });
  await saveCheckin(db, a, valid, new Date('2026-08-01T02:00:00Z'));
  const d = await buildDashboard(db, new Date('2026-08-01T02:00:00Z'));
